...
module darbitex_lp_locker::lock_tests {
    ...
    fun lock_then_view(
        ...
        let position_shares_before = pool::position_shares(position);

        let locker = lock::lock_position_and_get(user, position, 1000);

        assert!(lock::unlock_at_seconds(locker) == 1000, 100);
        assert!(object::object_address(&lock::position_of(locker)) == position_addr, 101);
        assert!(lock::position_shares(locker) == position_shares_before, 102);
        assert!(!lock::is_unlocked(locker), 103);
        assert!(object::owner(locker) == signer::address_of(user), 104);
        assert!(object::owner(position) != signer::address_of(user), 105);
    }
    ...
    fun redeem_after_unlock_returns_position(
        ...
        assert!(lock::is_unlocked(locker), 301);
        lock::redeem(user, locker);

        assert!(object::owner(position) == user_addr, 302);
        assert!(!object::object_exists<lock::LockedPosition>(locker_addr), 303);
    }

    #[test(aptos = @0x1, darbitex = @darbitex, seeder = @0xA11CE, user = @0xB0B)]
    fun redeem_position_returns_handle(
        aptos: &signer,
        darbitex: &signer,
        seeder: &signer,
        user: &signer,
        ...
        let position = setup(aptos, darbitex, seeder, user);
        let original_addr = object::object_address(&position);
        let locker = lock::lock_position_and_get(user, position, 1000);

        timestamp::update_global_time_for_test_secs(2000);
        let returned = lock::redeem_position(user, locker);
        assert!(object::object_address(&returned) == original_addr, 350);
        assert!(object::owner(returned) == signer::address_of(user), 351);
    }
    ...
    fun transferred_locker_new_owner_can_redeem(
        ...
        assert!(lock::unlock_at_seconds(locker) == 1000, 401);
    ...
    fun lock_transfers_position_away_from_user(
        ...
        let position = setup(aptos, darbitex, seeder, user);
        let user_addr = signer::address_of(user);
        assert!(object::owner(position) == user_addr, 500);

        lock::lock_position(user, position, 1000);

        assert!(object::owner(position) != user_addr, 501);
    }

    #[test]
    fun warning_contains_key_disclosure_terms() {
        let w = lock::read_warning();
        // Stay byte-anchored to terms used in source so silent renames break this test.
        assert!(contains(&w, b"unlock_at_seconds"), 600);
        assert!(contains(&w, b"timestamp::now_seconds"), 601);
        assert!(contains(&w, b"object::transfer"), 602);
        assert!(contains(&w, b"claim_fees_assets"), 603);
        assert!(contains(&w, b"redeem_position"), 604);
    }
