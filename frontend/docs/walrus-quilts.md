...
Last verified: **2026-04-24** (epoch 29, epoch duration 14 days).

| # | Shared Object ID | Blob ID (content hash) | Size | Exp. epoch | Exp. date | Resources |
|---|---|---|---|---|---|---|
| 11 | `0xd9f67e6629f3d49e66aa5e028ef96a090b00a6f1ce2e300d997348a5c8b4d4d5` | `BzX7NO5xQCuRwKJPkU9MzP490Pw0q7j2zRX3gFyMgw4` | 2.12 MiB | 34 | ~2026-07-06 | **7-nav merge + ONE absorb** — routes consolidated to Trade / Arbitrage / Liquidity / Earn / Tools / ONE / About (legacy flat routes removed). `/one` shell with 6 sub-tabs (Overview, Trove, SP, Redeem, Liquidate, About) + full write paths via `open_trove_pyth` / `add_collateral` / `close_trove` / `sp_{deposit,withdraw,claim}` / `redeem*_pyth` / `liquidate_pyth`. Pyth VAA fetched from Hermes v2 as `number[][]`. Auto-discovery of active troves via `account_transactions` → `user_transaction` JOIN (replaces deprecated `events` table). Proportional-to-holdings Disperse mode + inline per-row amount editor + weight display (FA balance / NFT count). ONE + USD1 added to TOKENS whitelist (icon PNGs), token order reshuffled APT / ONE / USDC / USDt / lzUSDC / lzUSDT / USD1 / DARBITEX. WARNING on /one/about rendered as 9 red-numbered blocks. `ws-resources.json` added — fixes SPA 404s on direct-URL hits (beta had it, Final didn't until now). Staking reward-pool dropdown auto-fetches all Darbitex pools via `pool_factory::get_all_pools` + `fetchFaMetadata`. About's TVL/volume also discovery-dynamic. |
