// D error codes from sources/D.move. Keep in sync with on-chain.
const D_ERRORS: Record<number, string> = {
  1: "Collateral below MCR (need ≥ 200% CR).",
  2: "Trove missing or already open.",
  3: "Debt below MIN_DEBT (0.1 D).",
...
//   "Move abort in 0x587c…::D: EDEBT_MIN(0x3): …"
//   "Move abort … code 3"
...
export function decodeDError(err: unknown): string {
  const msg = String((err as { message?: string })?.message ?? err);

  const decimal = msg.match(/(?:abort_code|code)\s*[":=]?\s*"?(\d+)"?/i);
  if (decimal) {
    const code = parseInt(decimal[1], 10);
    if (D_ERRORS[code]) return `${D_ERRORS[code]} (code ${code})`;
  }
